/**
 * 从 seed.js 的 EVENTS 生成「全删全增」SQL：drafts/full-rewrite-<生成日期>.sql
...
const OUT_NAME = `full-rewrite-${STAMP}.sql`;
...
lines.push(`--       （正史 ${main} 条·杂志特稿体 / 野史 ${dark} 条·红后终端体，含三对「检出/灭活」拆分线索）。`);
...
lines.push(`--   sqlite3 data\\chronicle.db ".read drafts/${OUT_NAME}"`);
lines.push(`--`);
...
lines.push(`--   sudo -u umbrella sqlite3 /opt/cursor-warco/data/chronicle.db ".read /opt/cursor-warco/drafts/${OUT_NAME}"`);
...
lines.push(`-- 三对检出/灭活线索应各 2 条，供给线应 2 条：`);
lines.push(`--   SELECT series, date, title FROM events WHERE series IN ('临期锁额','Team 席位差','假焚诀','供给线') ORDER BY series, date;`);
