...
作战室的「存活窗口」以标题带「（存活）」的档案为准。每周汇编战报时顺带执行：

1. 列出全部存活窗口（`/warroom` 或查库 `title LIKE '%（存活）%'`）。
2. 对每个窗口检索灭活信号：官方 changelog / 论坛公告提到修复；社区出现「patched / 不行了 / 对上了」的讨论。
3. 确认灭活 → 走 chronicler 立「·灭活」档案（同挂 series，原检出档案改题去「（存活）」补挂 series）→ 作战室与 /d/windows 自动更新。
